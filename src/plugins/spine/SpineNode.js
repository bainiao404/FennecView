...
export class spineNode extends node {
    ...
    setIsPremultiplied(t) {
        let node = this.node
        if (node && node.spineData && typeof node.spineData.setPremultiplied === 'function') {
    ...
    setSlotsAlpha(t) {
        let node = this.node
        if (node && node.skeleton && node.skeleton.slots[t[0]]) {
            node.skeleton.slots[t[0]].color.a = t[1] * 1
        }
    }
}
